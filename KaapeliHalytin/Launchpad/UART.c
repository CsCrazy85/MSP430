...
void TimerA_Delay(unsigned char seconds)
{
	mode = DELAY_MODE;
	delay = seconds;
	TACCTL1 &= ~(SCS + CM1 + CAP + CCIE); // UART dekonfigurointi

	BCSCTL3 = XCAP_3;   // asetetaan 12,5 pF:n sis‰iset kondensaattorit aktiiviseksi
	CCTL0 = CCIE;   // sallitaan vertailijan keskeytys
	CCR0 = 32767;    // ladataan vertailurekisterin arvo 1 sekunnin keskeytyst‰ varten
	TACTL = MC_1 + TACLR + ID_0 + TASSEL_1;      // k‰ynnistet‰‰n laskuri asetetaan jakaja 0, laskentamoodi vertaileva ja ACLK-kello.
...
void TimerA_msDelay(unsigned int ms)
{
	mode = DELAY_MODE;
	delay = ms;
	//TACCTL0 &= ~(SCS + CM1 + CAP + CCIE); // UART dekonfigurointi
	CCTL0 = CCIE;   // sallitaan vertailijan keskeytys
	CCR0 = 1000;    // ladataan vertailurekisterin arvo 1 millisekunnin keskeytyst‰ varten
	TACTL = MC_2 + TACLR + ID_0 + TASSEL_2;      // k‰ynnistet‰‰n laskuri asetetaan jakaja 0, laskentamoodi jatkuva ja SMCLK-kello.
